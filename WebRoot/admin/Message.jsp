	...
	$(function() {
		...
		$("#searchbtn").ligerButton({
			click : function() {
				...
				,
					{
				name:'message.admin_id' ,
				value: $("#admin_id").val()
				}
				,
					{
				name:'message.vehicle_id' ,
		...
		$("#maingrid").ligerGrid({
        ...
        
           {
        ...
        
			{
				...
				var str1 = '<a  class="l-icon-delete" onclick="operateById(\'删除 \',\'admin/deleteMessage.action\',' +row.messageId+ ',\'message.messageId\');">&nbsp;&nbsp;&nbsp;&nbsp;</a>';
				return str1;
				}
			...
			dataAction : 'server',
			url : "admin/Message_getByPage.action?type=1",
			detail : {
		...
		function f_showDetail(row, detailPanel) {
			...
			$.post("admin/searchMessageById.action", {
				'user.id' : row.id
			}, function(result)
...
//事件
	function itemclick(item) {
		...
		if (item.id) {
			...
			case "delete":
				...
				var url="admin/deleteMessageByList.action";
				var operate="删除";
				post(url,data,operate,'messageId');return;
	...
	function addOrEditAccount(operate){
		$("#save").click(function() {	
			...
			$.post("admin/Message_"+operate+".action", {
				'message.message_id' : $("#message_id").val(),
				'message.content' : $("#content").val(),
				'message.admin_id' : $("#admin_id").val(),
				'message.vehicle_id' : $("#vehicle_id").val(),
				'message.optime' : $("#optime").val(),
...
function edit(data){
		$.ligerDialog.open({
		  ...
		  $(data).each(function ()
                            {
                        $("#message_id").val(this.message_id); 
                        $("#content").val(this.content); 
                        $("#admin_id").val(this.admin_id); 
                        $("#vehicle_id").val(this.vehicle_id); 
                        $("#optime").val(this.optime); 
                        $("#type").val(this.type); 
                            });
		...
		<div id="target1" style="display: none margin:3px;">
			...
			<table width=100% align=center>
				<tbody>
					...
					<tr>
						<td>message_id:</td>
						<td><input type=text name=message.message_id id=message_id>
						</td>
						<td>中文</td>
					</tr>
					...
					<tr>
						<td>content:</td>
						<td><input type=text name=message.content id=content>
						</td>
						<td>中文</td>
					</tr>
					...
					<tr>
						<td>admin_id:</td>
						<td><input type=text name=message.admin_id id=admin_id>
						</td>
						<td>中文</td>
					</tr>
					...
					<tr>
						<td>vehicle_id:</td>
						<td><input type=text name=message.vehicle_id id=vehicle_id>
						</td>
						<td>中文</td>
					</tr>
					...
					<tr>
						<td>optime:</td>
						<td><input type=text name=message.optime id=optime>
						</td>
						<td>中文</td>
					</tr>
					...
					<tr>
						<td>type:</td>
						<td><input type=text name=message.type id=type>
						</td>
						<td>中文</td>
					</tr>

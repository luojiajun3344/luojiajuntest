			...
			var g = {
				onResize : function() {
					...
					if (g.accordion.parent()[0].tagName.toLowerCase() == "body") {
						...
						windowHeight -= parseInt(g.layout.parent()
								.css('paddingTop'));
						windowHeight -= parseInt(g.layout.parent()
								.css('paddingBottom'));
...
(function($) {
	...
	$.fn.ligerButton = function(p) {
		return this.each(function() {
			...
			g.button = $(this);
			if (!g.button.hasClass("l-btn"))
				g.button.addClass("l-btn");
			p.text && g.button.append("<span>" + p.text + "</span>");
			g.button
					.append('<div class="l-btn-l"></div><div class="l-btn-r"></div>');
	...
	$.fn.ligerComboBox = function(options) {
		this.each(function() {
			...
			var g = {
				...
				selectValue : function(value) {
					...
					if (p.tree) {
						...
						po.changeValue(value, text);
						var targetdata = value.toString().split(p.split);
						$("table.l-table-checkbox :checkbox", g.selectBox)
								.each(function() {
											this.checked = false;
										});
						for (var i = 0; i < targetdata.length; i++) {
				...
				setSelect : function() {
					...
					$('td:eq(' + g.select[0].selectedIndex + ')', g.selectBox)
							.each(function() {
								...
								if (g.select[0].selectedIndex != $(this)
										.attr('index')
										&& g.select[0].onchange) {
									g.select[0].selectedIndex = $(this)
											.attr('index');
									g.select[0].onchange();
								}
				...
				setData : function(data) {
					...
					$("table.l-table-nocheckbox td", g.selectBox).hover(
							function() {
								$(this).addClass("l-over");
							}, function() {
								$(this).removeClass("l-over");
				...
				setTree : function(tree) {
					...
					if (tree.checkbox != false) {
						tree.onCheck = function() {
							...
							po.changeValue(value.join(p.split), text
											.join(p.split));
						};
					} else {
			...
			var po = {
				dataInit : function() {
					...
					if (!p.isShowCheckBox && value != null) {
						$("table tr", g.selectBox).find("td:first").each(
								function() {
				...
				addClickEven : function() {
					...
					$(".l-table-nocheckbox td", g.selectBox).click(function() {
						if (p.onBeforeSelect
								&& !p.onBeforeSelect($(this).attr("value"),
										$(this).html())) {
							if (p.slide)
								g.selectBox.slideToggle("fast");
							else
					...
					if (isHide) {
						...
						if (p.slide) {
							...
							if (!g.tree
									...
									&& !p.isShowCheckBox
									&& $('td.l-selected', g.selectBox).length > 0) {
								var offSet = ($('td.l-selected', g.selectBox)
										.offset().top - g.selectBox.offset().top);
			...
			g.link.hover(function() {
						...
						if (p.onBeforeOpen && p.onBeforeOpen() == false)
							return false;
...
(function($) {
	...
	$.fn.ligerDateEditor = function(options) {
		return this.each(function() {
			...
			p = $.extend({}, $.ligerDefaults.DateEditor,
					$.ligerDefaults.DateEditorString, p);
			...
			var g = {
				bulidContent : function() {
					...
					var monthDayNum = new Date(nextYear, nextMonth - 1, 0)
							.getDate();
					// 当前上个月天数
					var prevMonthDayNum = new Date(g.currentDate.year,
							g.currentDate.month - 1, 0).getDate();

					g.buttons.btnMonth.html(p.monthMessage[g.currentDate.month
							- 1]);
					...
					$("tr", this.body.tbody).each(function(i, tr) {
						$("td", tr).each(function(j, td) {
							...
							if (g.selectedDate
								...
								$(td)
										.addClass("l-box-dateeditor-holiday")
										.removeClass("l-box-dateeditor-selected");
							} else {
								td.className = "";
							}
				...
				showDate : function() {
					if (!this.selectedDate)
						return;
					var dateStr = g.selectedDate.year + "/"
							+ g.selectedDate.month + "/" + g.selectedDate.date;
					this.currentDate.hour = parseInt(g.toolbar.time.hour.html());
					this.currentDate.minute = parseInt(g.toolbar.time.minute
							.html());
					if (p.showTime) {
...
(function($) {
	...
	$.ligerDialog.open = function(p) {
					...
					g.dialog.ligerResizable({
								onStopResize : function(current, e) {
									...
									if (current.diffTop != undefined) {
										...
										$(".l-dialog-content", g.dialog.body)
		...
		$(".l-dialog-buttons-inner", g.dialog)
				.append("<div class='l-clear'></div>");

		// 设置参数属性
		p.width && g.dialog.body.width(p.width - 26);
		if (p.height) {
			$(".l-dialog-content", g.dialog.body).height(p.height - 46
					- $(".l-dialog-buttons", g.dialog).height());
		}
		...
		return this.each(function() {
			...
			var g = {
				...
				applyDrag : function() {
					if (g.current.diffX) {
						g.target
								.css("left", (g.current.left + g.current.diffX));
					}
					if (g.current.diffY) {
...
(function($) {
	...
	$.ligerDefaults.Grid = {
		 ...
		 * 可折叠的+/-。例子给出的是默认参数 childrenName : 'children',
		 * //children的字段名。例子给出的是默认参数 isParent : function(rowData){
		 * //判断是否为父节点的（即显示-)判断函数。例子给出的是默认参数 var exist = 'children' in rowData;
		 * return exist; }, isExtend : function(rowData){ //判断是否张开。例子给出的是默认参数
		 * if('isextend' in rowData && rowData['isextend'] == false) return
		 * false; return true; } } json格式： Rows:[{tite:'11.11',chidren:[...] }]
		 */
	...
	$.ligerAddGrid = function(grid, p) {
		...
		gridhtmlarr
				.push("                    <div class='l-grid-editor'></div>");
		...
		gridhtmlarr
				.push("                        <div class='l-grid-header-inner'><table class='l-grid-header-table' cellpadding='0' cellspacing='0'><tbody><tr></tr></tbody></table></div>");
		gridhtmlarr.push("                    </div>");
		gridhtmlarr
				.push("                    <div class='l-grid-body l-scroll'>");
		gridhtmlarr.push("                    </div>");
		gridhtmlarr.push("                 </div>");
		gridhtmlarr.push("              </div>");
		gridhtmlarr.push("         </div>");
		...
		gridhtmlarr.push("                <div class='l-bar-separator'></div>");
		gridhtmlarr
				.push("                <div class='l-bar-group l-bar-right'><span class='l-bar-text'></span></div>");
		...
		var g = {
			...
			loadData : function(loadDataParm) {
				...
				if (p.dataAction == "server") {
					if (p.usePager) {
						param[p.pageParmName] = p.newPage;
						param[p.pagesizeParmName] = p.pageSize;
						// param.push({ name: p.pageParmName, value: p.newPage
						// });
						// param.push({ name: p.pagesizeParmName, value:
						// p.pageSize });
					}
					if (p.sortName) {
						param[p.sortnameParmName] = p.sortName;
						param[p.sortorderParmName] = p.sortOrder;
						// param.push({ name: p.sortnameParmName, value:
						// p.sortName });
						// param.push({ name: p.sortorderParmName, value:
						// p.sortOrder });
					}
				};
				...
				if (p.dataType == "local") {
					...
					if (clause)
						g.filteredData[p.root] = po.searchData(
						...
						if (p.dataAction == "server") {
							...
							if (clause)
								g.filteredData[p.root] = po.searchData(
										g.filteredData[p.root], clause);
							g.currentData = po
									.getCurrentPageData(g.filteredData);
						}
						setTimeout(function() {
									po.showData(g.currentData);
								}, 10);
			...
			getFormatDate : function(date, dateformat) {
				...
				if (!(date instanceof Date) && typeof(date) == 'object') {
					...
					date = new Date(date.year, date.date, date.hours,
			...
			deleteRow : function(rowParm) {
				...
				if (p.tree && g.hasChildren(rowObj)) {
					$("tbody:first > tr[parentrowid=" + rowid + "]", g.gridbody)
							.each(function() {
										g.deleteRow(this);
									});
				}
			...
			updateCell : function(cell, value, rowParm) {
				...
				var cellContent = po.getCellContent(rowData, rowindex, value,
						column, p.tree, row.attr("treelevel"));
			...
			addRow : function(rowdata, rowParm, isBefore, parentRow) {
				...
				if (parentRow) {
					...
					parentRowIsOpened = $(".l-grid-tree-link:first",
							parentRowObj).hasClass("l-grid-tree-link-open");
				}

				var olddatanumber = parentRowData
						? parentRowData[p.tree.childrenName].length
						: g.currentData[p.root].length;
				var rowObj = g.getRowObj(rowParm);
				var rowindex = rowObj
						? (parseInt($(rowObj).attr("rowindex")) + (isBefore
								? 0
								: 1))
						: olddatanumber;
				var rowHTML = po.getHtmlFromData([rowdata], p.tree, treelevel
								? treelevel
								: 1, parentrowid);
				var row = $(rowHTML);
				row.attr("rowindex", rowindex).removeClass("l-grid-row-last");
				if (parentRow && !parentRowIsOpened)
					row.hide();
				if (rowindex == olddatanumber) {
					...
					if (!p.usePager && !g.isTotalSummary()) {
						$("tbody:first > .l-grid-row:last", g.gridbody)
								.removeClass("l-grid-row-last");
						row.addClass("l-grid-row-last");
					}
				} else {
					if (parentRowData)
						parentRowData[p.tree.childrenName].splice(rowindex, 0,
								rowdata);
					else
						g.currentData[p.root].splice(rowindex, 0, rowdata);
					var selectexpr = p.tree ? "tr[parentrowid=" + parentrowid
							+ "][treelevel=" + treelevel + "]" : "tr";
				...
				if ($("tbody", g.gridbody).length == 0) {
					g.gridbody
							.html('<div class="l-grid-body-inner"><table class="l-grid-body-table" cellpadding=0 cellspacing=0><tbody></tbody></table></div>');
				}
				if (rowObj != undefined) {
					if (isBefore)
						$(rowObj).before(row);
			...
			updateRow : function(rowDom, newRowData) {
				...
				if (newRowData) {
					for (var columnname in newRowData) {
						if (columnname == p.statusName)
							continue;
						rowdata[columnname] = newRowData[columnname];
						var cellobj = $("> .l-grid-row-cell[columnname="
										+ columnname + "]", rowDom);
						if (cellobj.length == 0)
							continue;
			...
			getMulHeaderLevel : function() {
				...
				while (currentColumn) {
					level++;
					if (!currentColumn.columns || !currentColumn.columns.length)
						break;
			...
			getLevelColumns : function(column, level, columnLevel) {
				...
				for (var i = 0; column.columns && i < column.columns.length; i++) {
					var currentColumns = g.getLevelColumns(column.columns[i],
							level + 1, columnLevel);
				...
				if (p.dataAction == "local") {
					...
					if (p.sortName == columnName) {
						...
						g.sortedData[p.root].sort(function(data1, data2) {
									return po.compareData(data1, data2,
			...
			getCheckedRows : function() {
				...
				var rowdata = [];
				$("tbody:first > .l-checked", g.gridbody).each(
						function(i, row) {
							var rowid = $(row).attr("rowid");
			...
			getRowObj : function(rowParm) {
				if (typeof(rowParm) == "string" || typeof(rowParm) == "number") {
					return $(
							"tbody:first > .l-grid-row[rowid=" + rowParm + "]",
							g.gridbody).get(0);
				} else if (typeof(rowParm) == "object") {
					if (!rowParm)
						return null;
					if (typeof(rowParm.nodeType) != "undefined"
							&& rowParm.nodeType == 1)
						return rowParm;
					else {
						for (var p in g.records) {
							if (g.records[p] == rowParm)
								return $(
										"tbody:first > .l-grid-row[rowid=" + p
												+ "]", g.gridbody).get(0);
						}
					}
				}
			...
			toggleCol : function(columnparm, visible) {
				...
				if (typeof(columnparm) == "number") {
					...
					headercell = $(".l-grid-hd-cell[columnindex='" + columnparm
									+ "']", g.gridheader);
				} else if (typeof(columnparm) == "string") {
					headercell = $(".l-grid-hd-cell[columnname='" + columnparm
									+ "']", g.gridheader);
					if (!headercell)
						return;
				...
				if (visible) {
					headercell.show();
					$(".l-grid-row-cell[columnindex='" + columnindex + "']",
							g.gridbody).show();
				} else {
					headercell.hide();
					$(".l-grid-row-cell[columnindex='" + columnindex + "']",
			...
			isLeaf : function(rowParm) {
				...
				if (!rowObj)
					return;
				return !$("> td > div > .l-grid-tree-space:last", rowObj)
						.hasClass("l-grid-tree-link");
			},
			...
			appendRow : function(rowData, targetRow, nearRow, isBefore) {
				...
				if (nearRow) {
					g.addRow(rowData, nearRow, isBefore ? true : false,
							targetRowObj);
					return;
				}
				var rowid = $(targetRowObj).attr("rowid");
				var children = $(targetRowObj).nextAll("tr[parentrowid="
						+ rowid + "]").get();
				if (!children)
					return;
				if (children.length == 0)
					...
					g.addRow(rowData, children[children.length - 1], false,
							targetRowObj);
			},
			upgrade : function(targetRow) {
				...
				$("> td > div > .l-grid-tree-space:last", targetRow)
		...
		var po = {
			...
			initBuildGridHeader : function() {
				...
				for (var level = 1; level <= maxLevel - 1; level++) {
					...
					$(mulHeaders).each(function(i, item) {
						...
						$(".l-grid-hd-cell-text", $headerCell)
								.html(item.display);
						tr.append($headerCell);
					});
				}
				...
				$(g.columns).each(function(i, item) {
					...
					if (typeof(colwidth) == "string"
							&& colwidth.indexOf('%') > 0) {
						item.width = colwidth = parseInt(parseInt(colwidth)
								* 0.01
								* (g.gridbody.width() - g.columns.length));
					}
			...
			initBuildPopup : function() {
				$("tr:last .l-grid-hd-cell", g.gridheader).each(
						function(i, td) {
							if ($(this).hasClass("l-grid-hd-cell-detail"))
								return;
							var isAllowHide = $(this).attr("isAllowHide");
							if (isAllowHide != undefined
									&& isAllowHide.toLowerCase() == "false")
								return;
				...
				// 创建 显示/隐藏 列
				$(".l-grid-hd-cell", g.gridheader).bind("contextmenu",
						function(e) {
							if (g.colresize)
								return true;
							if (!p.allowHideColumn)
			...
			initEven : function() {
				...
				$(".l-grid-hd-cell", g.gridheader).hover(function() {
							...
							if (g.colresize)
								return false; // 如果正在调整列宽
						});
				$(".l-grid-hd-cell-text", g.gridheader).click(function(e) {
					var obj = (e.target || e.srcElement);
					var row = $(this).parent().parent();
					if (!row.attr("columnname"))
						return;
					if (g.colresize)
						return false; // 如果正在调整列宽
					if (!p.enabledSort)
						return;
					if (row.attr("isSort") != undefined
							&& row.attr("isSort").toLowerCase() == "false")
						return;
					if (g.isDataChanged && !confirm(p.isContinueByDataChanged))
						return false;
					...
					if (sort.length > 0) {
						if (sort.hasClass("l-grid-hd-cell-sort-asc")) {
							...
							row.removeClass("l-grid-hd-cell-asc")
							...
							row.removeClass("l-grid-hd-cell-desc")
					...
					g.gridheader.mousemove(function(e) {
						...
						for (var i = 0; i < g.headers.length; i++) {
							if (g.headers[i].width)
								currentLeft += g.headers[i].width + 1;
							if (g.headers[i].isdetail
									|| g.headers[i].ischeckbox)
								continue;
							if (posLeft >= currentLeft - 2
									- g.gridbody[0].scrollLeft
									&& posLeft <= currentLeft + 2
											- g.gridbody[0].scrollLeft) {
								$('body').css({
											cursor : 'e-resize'
										});
				...
				$(grid).click(function(e) {
					...
					if (obj.tagName.toLowerCase() == "span"
							&& $(obj).hasClass("l-grid-row-cell-detailbtn")) {
						var row = $(obj).parent().parent().parent();
						// 确保不是在内嵌表格点击的
						if (row.parent().parent()[0] != $("table:first",
								g.gridbody)[0])
							return;
						...
						if ($(obj).hasClass("l-open")) {
							...
							detailRowInner.parent().attr("colSpan",
									g.headers.length).width(g.gridtablewidth
									- 1);
							row.after(detailRow);
					...
					if (obj.tagName.toLowerCase() == "div"
							&& $(obj).hasClass("l-grid-hd-cell-btn-checkbox")) {
						var row = $(obj).parent().parent().parent();
						var uncheck = row.hasClass("l-checked");
						if (p.onBeforeCheckAllRow) {
							if (p.onBeforeCheckAllRow(!uncheck, grid) == false)
								return false;
						}
						if (uncheck) {
							row.removeClass("l-checked");
							$("tbody:first > tr.l-grid-row", g.gridbody)
					...
					if (obj.tagName.toLowerCase() == "div"
							|| $(obj).hasClass("l-grid-row-cell-inner")
							|| $(obj).hasClass("l-grid-row-cell")) {
				...
				$(".l-bar-button", g.toolbar).hover(function() {
					...
					if ($(this).hasClass("l-bar-btnfirst")) {
						if (p.onToFirst && p.onToFirst(grid) == false)
							return false;
						g.changePage('first');
					} else if ($(this).hasClass("l-bar-btnprev")) {
						if (p.onToPrev && p.onToPrev(grid) == false)
							return false;
						g.changePage('prev');
					} else if ($(this).hasClass("l-bar-btnnext")) {
						if (p.onToNext && p.onToNext(grid) == false)
							return false;
						g.changePage('next');
					} else if ($(this).hasClass("l-bar-btnlast")) {
						if (p.onToLast && p.onToLast(grid) == false)
							return false;
						g.changePage('last');
					} else if ($(this).hasClass("l-bar-btnload")) {
						if ($("span", this).hasClass("l-disabled"))
							return false;
						if (p.onReload && p.onReload(grid) == false)
							return false;
						if (g.isDataChanged
								&& !confirm(p.isContinueByDataChanged))
							return false;
			...
			showData : function(data) {
				...
				if (!data || !data[p.root])
					return;
				if (p.onBeforeShowData
						&& p.onBeforeShowData(grid, data) == false) {
					return false;
				}
				...
				// 清空数据
				g.gridbody.html("");
				// $(".l-grid-row,.l-grid-detailpanel,.l-grid-totalsummary",
				// g.gridbody).remove();
				// 加载数据
				...
				if (p.groupColumnName) // 启用分组模式
				{
					...
					$(data[p.root]).each(function(i, item) {
								...
								var valueIndex = $.inArray(groupColumnValue,
										groups);
								if (valueIndex == -1) {
					...
					$(groupsdata).each(function(i, item) {
						gridhtmlarr
								.push('<tr class="l-grid-grouprow" rowcount="'
										+ item.length + '">');
						gridhtmlarr.push('<td colSpan="' + g.headers.length
						...
						// 汇总
						if (g.isTotalSummary())
							gridhtmlarr.push(po.getTotalSummaryHtml(item,
									"l-grid-totalsummary-group"));
					});
				} else if (p.tree)// 启用分页模式
				{
					if (!p.tree.columnName)
						p.tree.columnName = "name";
					if (!p.tree.childrenName)
						p.tree.childrenName = "children";
					if (!p.tree.isParent)
					...
					if (!p.tree.isExtend)
						p.tree.isExtend = function(rowData) {
					...
					gridhtmlarr.push(po
							.getHtmlFromData(data[p.root], p.tree, 1));
				} else {
				...
				po.onResize();
				// 分组 - 事件
				$("tbody:first > .l-grid-grouprow", g.gridbody).each(
						function() {
							var grouprow = $(this);
							$(".l-grid-group-togglebtn", grouprow).click(
									function() {
										var opening = true;
										if ($(this)
												.hasClass("l-grid-group-togglebtn-close"))
											$(this)
													.removeClass("l-grid-group-togglebtn-close");
										else {
											opening = false;
											$(this)
													.addClass("l-grid-group-togglebtn-close");
										}
										var currentRow = grouprow
												.next(".l-grid-row,.l-grid-totalsummary-group");
										while (true) {
											if (currentRow.length == 0)
												break;
											if (opening)
				...
				// 表体 - 行经过事件
				$("tbody:first > .l-grid-row", g.gridbody).each(function() {
							po.setRowEven(this);
						});
				if (p.totalRender) {
			...
			onClick : function(e) {
				...
				if (g.grideditor.editingCell) {
					if (tagName == 'html' || tagName == 'body'
							|| $(obj).hasClass("l-grid-body")
							|| $(obj).hasClass("l-grid-row")) {
						g.endEdit(true);
					}
				}
				if (p.allowHideColumn) {
			...
			getHtmlFromData : function(dataArray, tree, level, parentrowid) {
				...
				$(dataArray).each(function(i, item) {
					...
					if (!p.usePager && i == rowlenth - 1 && !g.isTotalSummary())
						gridhtmlarr
								.push('<tr class="l-grid-row l-grid-row-last');
					else
					...
					gridhtmlarr.push('" ');
					if (tree)
						gridhtmlarr.push(" treelevel= " + level);
					if (p.rowAttrRender)
						gridhtmlarr.push(p.rowAttrRender(item, i));
					...
					$(g.headers).each(function(headerCellIndex, headerInfor) {
						...
						if (this.ischeckbox) {
							gridhtmlarr
									.push('<td class="l-grid-row-cell l-grid-row-cell-checkbox" style="width:'
											+ this.width
											+ 'px"><div class="l-grid-row-cell-inner"><span class="l-grid-row-cell-btn-checkbox"></span></div></td>');
							return;
						}
						// 如果是明细列(系统列)
						else if (this.isdetail) {
							gridhtmlarr
									.push('<td class="l-grid-row-cell l-grid-row-cell-detail" style="width:'
											+ this.width
											+ 'px"><div class="l-grid-row-cell-inner"><span class="l-grid-row-cell-detailbtn"></span></div></td>');
							return;
						}
						...
						if (!this.islast)
							gridhtmlarr
									.push('<td class="l-grid-row-cell" columnindex="'
											+ this.columnindex + '" ');
						else
							gridhtmlarr
									.push('<td class="l-grid-row-cell l-grid-row-cell-last" columnindex="'
											+ this.columnindex + '" ');
						if (this.columnname)
							gridhtmlarr.push('columnname="' + this.columnname
									+ '"');
						...
						if (p.fixedCellHeight)
							gridhtmlarr
									.push('><div class="l-grid-row-cell-inner l-grid-row-cell-inner-fixedheight" ');
						else
							gridhtmlarr
									.push('><div class="l-grid-row-cell-inner" ');
						if (typeof(colwidth) == "string"
								&& colwidth.indexOf('%') > 0) {
							gridhtmlarr.push(' style = "width:95%; ');
						} else {
							gridhtmlarr.push(' style = "width:'
									+ parseInt(colwidth - 8) + 'px; ');
						}
						...
						{
							var splitvalue = (this.columnname).split('.');
							var result1 = item;
							for (var i = 0; i < splitvalue.length; i++) {
								result1 = result1[splitvalue[i]];
								if (result1 == null) {
									break;
								}
							}
						...
						var content = po.getCellContent(item, i, result1,
								column, tree, level);
					...
					if (tree && tree.isParent(item)) {
						...
						if (childrenData)
							gridhtmlarr.push(po.getHtmlFromData(childrenData,
									tree, level + 1, rowid));
					}
				});
			...
			getCellContent : function(rowData, rowindex, value, column, tree,
					level) {
				var content = "";
				if (column.render) {
				...
				// luojiajunmodify
					...
					if (value instanceof Date) {
						if (column.format)
							...
							content = g.getFormatDate(value, p.dateFormat);
					}
			...
			setRowEven : function(rowobj) {
				$(rowobj).bind("contextmenu", function(e) {
					...
					if (p.onRClickToSelect)
						$(this).addClass("l-selected").siblings(".l-selected")
								.removeClass("l-selected");
					if (p.onContextmenu) {
				...
				$(rowobj).hover(function() {
					...
					if (p.checkbox) {
						...
						var selectRowButtonOnly = p.selectRowButtonOnly
								? true
								: false;
						if (p.enabledEdit)
						...
						if (!selectRowButtonOnly
								|| $(srcObj)
										.hasClass("l-grid-row-cell-btn-checkbox")) {
							var row = $(this);
							var index = row.attr('rowindex');
							var rowid = row.attr("rowid");
							var uncheck = row.hasClass("l-checked");
							if (p.onBeforeCheckRow) {
								if (p.onBeforeCheckRow(!uncheck, g
												.getRow(rowid), index, row[0]) == false)
									return false;
							}
							if (uncheck)
								row.removeClass("l-checked");
								...
								row.addClass("l-checked");
							p.onCheckRow
									&& p.onCheckRow(!uncheck, g.getRow(rowid),
											index, row[0]);
						}
						if (!p.enabledEdit)
							return;
					...
					if ($(this).hasClass("l-selected")) {
						...
						$(this).siblings(".l-selected").each(function() {
							if (p.allowUnSelectRow
									|| $(this).hasClass("l-selected-again"))
								g.endEdit();
							$(this).removeClass("l-selected l-selected-again");
						});
						$(this).addClass("l-selected");
			...
			applyEditor : function(obj) {
				...
				g.grideditor.css({
							left : left,
							top : top,
							width : $(rowcell).css('width'),
							height : $(rowcell).css('height')
						}).html("");
				g.grideditor.editingCell = null;
				if (column.editor && column.editor.type == 'date') {
					...
					if (column.editor.p)
						options = $.expend({},
								typeof(column.editor.p) == 'function'
										? column.editor.p(rowdata, rowindex,
												currentdata, column)
										: column.editor.p, options);
					...
					if (column.editor.dataValueField)
						options.valueField = column.editor.dataValueField;
					else if (column.editor.valueColumnName)
						options.valueField = column.editor.valueColumnName;
					if (column.editor.dataDisplayField)
						options.displayField = options.textField = column.editor.dataDisplayField;
					else if (column.editor.displayColumnName)
						options.displayField = options.textField = column.editor.displayColumnName;
					if (column.editor.valueColumnName)
					...
					g.grideditor.show();
						...
						&& (column.editor.type == 'int'
								|| column.editor.type == 'float' || column.editor.type == 'spinner')) {
					var $inputText = $("<input type='text'/>");
					g.grideditor.append($inputText);
					$inputText.attr({
								style : 'border:#6E90BE'
					...
					g.grideditor.show();
						...
						&& (column.editor.type == 'string' || column.editor.type == 'text')) {
					...
					$inputText.ligerTextBox({
								...
								onChangeValue : function(newValue) {
									$(rowcell)
											.addClass("l-grid-row-cell-edited");
									if (column.editor.onChange)
										column.editor.onChange(rowcell,
												newValue);
									editParm.value = newValue;
									if (po.checkEditAndUpdateCell(editParm)) {
										if (column.editor.onChanged)
					...
					g.grideditor.show();
						...
						&& (column.editor.type == 'chk' || column.editor.type == 'checkbox')) {
			...
			checkEditAndUpdateCell : function(editParm) {
				if (p.onBeforeSubmitEdit) {
					if (!p.onBeforeSubmitEdit(editParm))
						return false;
				}
			...
			getCurrentPageData : function(jsonObj) {
				...
				data[p.record] = jsonObj[p.root].length
						? jsonObj[p.root].length
						: 0;
				if (!p.newPage)
					p.newPage = 1;
				for (i = (p.newPage - 1) * p.pageSize; i < jsonObj[p.root].length
			...
			compareData : function(data1, data2, columnName, columnType) {
				...
				switch (columnType) {
					case "int" :
						return parseInt(data1[columnName]) < parseInt(data2[columnName])
								? -1
								: parseInt(data1[columnName]) > parseInt(data2[columnName])
										? 1
										: 0;
					case "float" :
						return parseFloat(data1[columnName]) < parseFloat(data2[columnName])
								? -1
								: parseFloat(data1[columnName]) > parseFloat(data2[columnName])
										? 1
										: 0;
					case "string" :
						return data1[columnName]
								.localeCompare(data2[columnName]);
					case "date" :
						return data1[columnName] < data2[columnName]
								? -1
								: data1[columnName] > data2[columnName] ? 1 : 0;
				}
						...
						if (column.totalSummary.render) {
							...
							if (isExist('sum'))
								totalsummaryArr.push("<div>Sum="
										+ sum.toFixed(2) + "</div>");
							if (isExist('count'))
								totalsummaryArr.push("<div>Count=" + count
										+ "</div>");
							if (isExist('max'))
			...
			bulidTotalSummary : function() {
				...
				if (!g.currentData || g.currentData[p.root].length == 0)
					return false;
				$("tbody:first", g.gridbody).append(po
						.getTotalSummaryHtml(g.currentData[p.root]));
			},
			buildPager : function() {
			...
			onResize : function() {
				if (p.height && p.height != 'auto') {
					...
					if (typeof(p.height) == "string"
						...
						if (p.InWindow
								|| gridparent[0].tagName.toLowerCase() == "body")
							h -= ($(grid).offset().top - parseInt($('body')
									.css('paddingTop')));
					} else {
						h = parseInt(p.height);
					}
			...
			dragEnd : function(e) {
				if (g.colresize) {
					...
					newwidth = newwidth < mincolumnwidth
							? mincolumnwidth
							: newwidth;
					...
					$("div:first", g.gridheader).width(g.gridtablewidth + 40);
					$("div:first", g.gridbody).width(g.gridtablewidth);
					$('.l-grid-hd-cell[columnindex=' + columnindex + ']',
							g.gridheader).css('width', newwidth);
					$(
							'tbody:first > .l-grid-row > td[columnindex='
									+ columnindex
									+ '],tbody:first > .l-grid-totalsummary > td[columnindex='
									+ columnindex + ']', g.gridbody).each(
							function() {
		...
		if (grid.id == undefined)
			grid.id = "LigerUI_" + new Date().getTime();
			...
			var g = {
				init : function() {
					...
					$(".l-layout-header-toggle", g.layout).hover(function() {
								...
								$(this)
										.removeClass("l-layout-header-toggle-over");
				...
				setCollapse : function() {
					...
					g.leftCollapse.hover(function() {
								...
								$(this)
										.removeClass("l-layout-collapse-left-over");
							});
					g.leftCollapse.toggle.hover(function() {
								$(this)
										.addClass("l-layout-collapse-left-toggle-over");
							}, function() {
								$(this)
					...
					g.rightCollapse.hover(function() {
								$(this)
										.addClass("l-layout-collapse-right-over");
							}, function() {
								$(this)
										.removeClass("l-layout-collapse-right-over");
							});
					...
					if (g.xresize && g.xresize.diff != undefined) {
						if (g.dragtype == 'leftresize') {
							...
							if (g.center)
								...
								g.right.width(g.left.width() - g.xresize.diff)
										.css({
											left : parseInt(g.right.css('left'))
													+ g.xresize.diff
										});
		...
		return this.each(function() {
			...
			var g = {
				addItem : function(item) {
					...
					// item.icon && $("span:first",ditem).wrap('<div
					// class="l-menu-item-icon l-icon-' + item.icon +
					// '"></div>');
					item.click && ditem.click(function() {
								item.click(item);
							});
...
(function($) {
	...
	$.ligerMessageBox.show = function(p) {
		...
		var g = {
			...
			applyDrag : function() {
				if (p.isDrag && $.fn.ligerDrag)
					messageBox.ligerDrag({
								handler : '.l-messagebox-title'
							});
			},
			setImage : function() {
				if (p.type) {
					if (p.type == 'success' || p.type == 'donne') {
						$(".l-messagebox-image", messageBox)
								.addClass("l-messagebox-image-donne").show();
						$(".l-messagebox-content", messageBox).css({
									paddingLeft : 64,
									paddingBottom : 30
								});
					} else if (p.type == 'error') {
		...
		if (p.buttons) {
			$(p.buttons).each(function(i, item) {
				...
				item.width && btn.width(item.width);
				item.onclick && btn.click(function() {
							item.onclick(item, i, messageBox)
						});
			});
			$(".l-messagebox-buttons-inner", messageBox)
					.append("<div class='l-clear'></div>");
		}
		var boxWidth = messageBox.width();
		var sumBtnWidth = 0;
		$(".l-messagebox-buttons-inner .l-messagebox-btn", messageBox).each(
				function() {
					sumBtnWidth += $(this).width();
				});
		$(".l-messagebox-buttons-inner", messageBox).css({
		...
		$(".l-messagebox-btn", messageBox).hover(function() {
					...
					$(".l-messagebox-btn-l", this)
							.addClass("l-messagebox-btn-l-over");
					$(".l-messagebox-btn-r", this)
							.addClass("l-messagebox-btn-r-over");
				}, function() {
					$(this).removeClass("l-messagebox-btn-over");
					$(".l-messagebox-btn-l", this)
							.removeClass("l-messagebox-btn-l-over");
					$(".l-messagebox-btn-r", this)
							.removeClass("l-messagebox-btn-r-over");
				});
	...
	function preLoadImage() {
		...
		var imageArr = ['box/box-btn-done.gif', 'box/box-btn-error.gif',
				'box/box-btn-l.gif', 'box/box-btn-l-over.gif',
				'box/box-btn-question.gif', 'box/box-btn-over.gif',
				'box/box-righttop.gif', 'box/box-lefttop.gif',
				'box/box-top.gif', 'box/box-btn-r.gif',
				'box/box-btn-r-over.gif', 'box/box-btn-warn.gif',
				'box/box-close.gif', 'box/box-close-over.gif',
				'box/tabs-item-left-bg.gif'];
					...
					var g = {
						doclick : function() {
							...
							if (g.input[0].form)
								...
								formEle = document;
							$("input:radio[name=" + g.input[0].name + "]",
									formEle).not(g.input).trigger("change");
							return false;
						}
					};
					g.input = $(this);
		...
		return this.each(function() {
			...
			var g = {
				init : function() {
					...
					$("> .l-resizable-h-c , > .l-resizable-f-c", g.target)
							.width(g.target.width());
					$("> .l-resizable-c-l , > .l-resizable-c-r", g.target)
	...
	$.fn.ligerTab = function(p) {
		...
		this.each(function() {
			...
			var g = {
				...
				setTabButton : function() {
					...
					if (sumwidth > mainwidth) {
						...
						$(".l-tab-links-left,.l-tab-links-right", g.tab.links)
								.remove();
						return false;
					}
				},
				...
				moveToNextTabItem : function() {
					...
					for (var i = tabItems.length - 1; i >= 0; i--) {
						var currentItemLeft = sumwidth - mainwidth + btnWitdth
								+ 2;
				...
				moveToLastTabItem : function() {
					...
					var mainwidth = g.tab.width();
					if (sumwidth > mainwidth) {
						var btnWitdth = $(".l-tab-links-right", g.tab.links)
								.width();
						g.tab.links.ul.animate({
									left : -1
											* (sumwidth - mainwidth + btnWitdth + 2)
								});
					}
				},
				// 判断tab是否存在
				...
				addTabItemEvent : function(tabitem) {
					...
					$(".l-tab-links-item-close", tabitem).hover(function() {
								...
								$(this)
										.removeClass("l-tab-links-item-close-over");
							}).click(function() {
				...
				removeTabItem : function(tabid) {
					if (p.onBeforeRemoveTabItem
						...
						return false;
					var currentIsSelected = $("li[tabid=" + tabid + "]",
							g.tab.links.ul).hasClass("l-selected");
				...
				getTabidList : function(notabid, noclose) {
					...
					$("> li", g.tab.links.ul).each(function() {
						if ($(this).attr("tabid")
								...
								&& (!noclose || $(".l-tab-links-item-close",
										this).length > 0)) {
				...
				reload : function(tabid) {
					$(".l-tab-content-item[tabid=" + tabid + "] iframe",
							g.tab.content).each(function(i, iframe) {
				...
				onResize : function() {
					...
					if (g.tab.parent()[0].tagName.toLowerCase() == "body") {
						var windowHeight = $(window).height();
						windowHeight -= parseInt(g.tab.parent()
								.css('paddingTop'));
						windowHeight -= parseInt(g.tab.parent()
								.css('paddingBottom'));
						g.height = p.heightDiff + windowHeight
			...
			$("> div", g.tab.content).each(function(i, box) {
				...
				if (!$(box).hasClass("l-tab-content-item"))
					$(box).addClass("l-tab-content-item");
			});
			// init
		...
		this.each(function() {
			...
			tip.css({
						left : $(this).offset().left + $(this).width()
								+ p.distanceX,
						top : $(this).offset().top + p.distanceY
					}).show();
...
(function($) {
	...
	$.fn.ligerToolBar = function(p) {
		this.each(function() {
			...
			g.toolBar = $(this);
			if (!g.toolBar.hasClass("l-toolbar"))
	...
	$.fn.ligerTree = function(p) {
		...
		this.each(function() {
			...
			var g = {
				...
				getChecked : function() {
					if (!p.checkbox)
						return null;
					var nodes = [];
					$(".l-checkbox-checked", g.tree).parent().parent("li")
							.each(function() {
				...
				getSelected : function() {
					...
					if (node.target) {
						var treedataindex = parseInt($(node.target)
								.attr("treedataindex"));
						node.data = po.getDataNodeByTreeDataIndex(g.data,
								treedataindex);
						return node;
					}
					return null;
				...
				upgrade : function(treeNode) {
					...
					$("." + po.getChildNodeClassName(), treeNode).each(
							function() {
								$(this).removeClass(po.getChildNodeClassName())
										.addClass(po
				...
				demotion : function(treeNode) {
					...
					$("." + po.getParentNodeClassName(true), treeNode).each(
							function() {
								$(this).removeClass(po
										.getParentNodeClassName(true))
				...
				remove : function(treeNode) {
					var treedataindex = parseInt($(treeNode)
							.attr("treedataindex"));
					var treenodedata = po.getDataNodeByTreeDataIndex(g.data,
							treedataindex);
					if (treenodedata)
				...
				update : function(domnode, newnodedata) {
					var treedataindex = parseInt($(domnode)
							.attr("treedataindex"));
					nodedata = po.getDataNodeByTreeDataIndex(g.data,
							treedataindex);
					for (var attr in newnodedata) {
						nodedata[attr] = newnodedata[attr];
						if (attr == p.textFieldName) {
							$("> .l-body > span", domnode)
									.text(newnodedata[attr]);
						}
					}
				},
				// 增加节点集合
				append : function(parentNode, newdata) {
					if (p.onBeforeAppend
							&& p.onBeforeAppend(parentNode, newdata) == false)
						return false;
					...
					if (!parentNode)// 增加到根节点
					{
						...
						var gridhtmlarr = po.getTreeHTMLByData(newdata, 1, [],
								true);
					...
					if ($("> .l-children > li:last", treeitem).length > 0)
						po.setTreeItem(
								$("> .l-children > li:last", treeitem)[0], {
									isLast : false
								});

					var isLast = [];
					for (var i = 1; i <= outlineLevel - 1; i++) {
						var currentParentTreeItem = $(g.getParentTreeItem(
								parentNode, i));
				...
				cancelSelect : function(domNode) {
					...
					p.onCancelSelect && p.onCancelSelect({
								data : treenodedata,
								target : treeitem[0]
							});
				},
				...
				getDataByID : function(id) {
					...
					$("li", g.tree).each(function() {
						...
						var treenodedata = po.getDataNodeByTreeDataIndex(
								g.data, treedataindex);
						if (treenodedata[p.idFieldName].toString() == id
								.toString()) {
							data = treenodedata;
						}
					});
			...
			var po = {
				...
				getDataNodeByTreeDataIndex : function(data, treedataindex) {
					for (var i = 0; i < data.length; i++) {
						if (data[i].treedataindex == treedataindex)
							return data[i];
						if (data[i].children) {
							var targetData = po.getDataNodeByTreeDataIndex(
				...
				setTreeItem : function(treeNode, options) {
					...
					if (options.isLast != undefined) {
						if (options.isLast == true) {
							...
							$(".l-children li", treeItem).find(".l-box:eq("
									+ (outlineLevel - 1) + ")")
									.removeClass("l-line").addClass("l-line");
						}
					}
				},
				...
				getTreeHTMLByData : function(data, outlineLevel, isLast,
						isExpand) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						if (g.hasChildren(data[i])) {
							...
							p.parentIcon
									&& !isExpandCurrent
									&& treehtmlarr.push('<div class="l-box '
											+ po.getParentNodeClassName()
											+ '"></div>');
							p.parentIcon
									&& isExpandCurrent
									&& treehtmlarr.push('<div class="l-box '
											+ po.getParentNodeClassName(true)
											+ '"></div>');
						} else {
							if (isLastCurrent)
								treehtmlarr
							...
							p.childIcon
									&& treehtmlarr.push('<div class="l-box '
											+ po.getChildNodeClassName()
											+ '"></div>');
						}

						treehtmlarr.push('<span>' + data[i][p.textFieldName]
								+ '</span></div>');
						if (g.hasChildren(data[i])) {
							var isLastNew = [];
							...
							treehtmlarr.push(po.getTreeHTMLByData(
									...
									isLastNew, isExpandCurrent).join(''));
						}
						treehtmlarr.push('</li>');
					}
					treehtmlarr.push("</ul>");
				...
				getDataByTreeHTML : function(treeDom) {
					...
					$("> li", treeDom).each(function(i, item) {
						...
						data[dataindex][p.textFieldName] = $("> span,> a", this)
								.html();
						for (var j = 0; j < g.sysAttribute.length; j++) {
							if ($(this).attr(g.sysAttribute[j]))
								data[dataindex][g.sysAttribute[j]] = $(this)
						...
						if ($("> ul", this).length > 0) {
							data[dataindex].children = po.getDataByTreeHTML($(
									"> ul", this));
						}
					});
					return data;
				...
				setTreeEven : function() {
					p.onContextmenu && g.tree.bind("contextmenu", function(e) {
								...
								var treedataindex = parseInt(treeitem
										.attr("treedataindex"));
								var treenodedata = po
										.getDataNodeByTreeDataIndex(g.data,
												treedataindex);
								return p.onContextmenu({
											data : treenodedata,
											target : treeitem[0]
					...
					g.tree.click(function(e) {
						...
						if (!$(obj).hasClass("l-checkbox")) {
							if ($(">div:first", treeitem)
								...
								if (p.onBeforeSelect && p.onBeforeSelect({
											data : treenodedata,
											target : treeitem[0]
										}) == false)
									return false;
						...
						if ($(obj).hasClass("l-checkbox")) {
							if (p.autoCheckboxEven) {
								...
								// 状态：未完全选中
										...
										.hasClass("l-checkbox-incomplete")) {
						...
						// 状态：已经张开
							...
							if (p.onBeforeCollapse && p.onBeforeCollapse({
										data : treenodedata,
										target : treeitem[0]
									}) == false)
								return false;
							...
							$("> div ." + po.getParentNodeClassName(true),
									treeitem).removeClass(po
									.getParentNodeClassName(true)).addClass(po
						...
						// 状态：没有张开
							...
							if (p.onBeforeExpand && p.onBeforeExpand({
										data : treenodedata,
										target : treeitem[0]
									}) == false)
								return false;
							...
							var callback = function() {
								p.onExpand && p.onExpand({
											data : treenodedata,
											target : treeitem[0]
										});
							};
							if (p.slide) {
								$("> .l-children", treeitem).slideToggle(
										'fast', callback);
							} else {
								$("> .l-children", treeitem).toggle();
								callback();
				...
				setParentCheckboxStatus : function(treeitem) {
					...
					var isCheckedComplete = $(".l-checkbox-unchecked", treeitem
									.parent()).length == 0;
					// 当前同级别或低级别的节点是否都没有选中
					var isCheckedNull = $(".l-checkbox-checked", treeitem
									.parent()).length == 0;
					...
					var getItem = function(container, id) // 获取数据项(为空时表示没有插入)
					{
						...
						for (var i = 0; i < container.length; i++) {
							...
							if (container[i].children) {
								var finditem = getItem(container[i].children,
										id);
								if (finditem)
									return finditem;
							}
...
(function($) {
	...
	$.fn.ligerApplyWindow = function(p) {
		return this.each(function() {
			...
			var g = {};
			g.window = $('<div class="l-window"><div class="l-window-header"><div class="l-dialog-title">提示</div><div class="l-window-header-buttons"><div class="l-window-toggle"></div><div class="l-window-max"></div><div class="l-window-close"></div><div class="l-clear"></div></div><div class="l-window-header-inner"></div></div><div class="l-window-content"></div></div>');
			...
			p.title
					&& $(".l-window-header-inner", g.window.header)
							.html(p.title);
			p.framename
			...
			if ($.fn.ligerResizable) {
				...
				g.window.ligerResizable({
							onStartResize : function() {
								$.ligerWindow.switchWindow(g.window[0]);
								if ($(".l-window-max", g.window)
										.hasClass("l-window-regain")) {
	...
	$.ligerWindow.switchWindow = function(window) {
		$(window).css("z-index", "101").siblings(".l-window").css("z-index",
				"100");
	};

...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#searchbtn").ligerButton({
			click : function() {
				...
				gridManager.setOptions({
					parms : [ 
										{
				name:'rolemodule.role_id' ,
				value: $("#role_id").val()
				}
				,
					{
				name:'rolemodule.module_id' ,
				value: $("#module_id").val()
				}
		...
		$("#maingrid").ligerGrid({
        ...
        
           {
        ...
        
			{
				...
				var str1 = '<a  class="l-icon-delete" onclick="operateById(\'删除 \',\'admin/deleteRolemodule.action\',' +row.rolemoduleId+ ',\'rolemodule.rolemoduleId\');">&nbsp;&nbsp;&nbsp;&nbsp;</a>';
				return str1;
				}
			...
			dataAction : 'server',
			url : "admin/Rolemodule_getByPage.action?type=1",
			detail : {
		...
		function f_showDetail(row, detailPanel) {
			...
			$.post("admin/searchRolemoduleById.action", {
				'user.id' : row.id
			}, function(result)
...
//事件
	function itemclick(item) {
		...
		if (item.id) {
			...
			case "delete":
				...
				var url="admin/deleteRolemoduleByList.action";
				var operate="删除";
				post(url,data,operate,'rolemoduleId');return;
	...
	function addOrEditAccount(operate){
		$("#save").click(function() {	
			...
			$.post("admin/Rolemodule_"+operate+".action", {
				'rolemodule.role_id' : $("#role_id").val(),
				'rolemodule.module_id' : $("#module_id").val()
		...
		<div id="target1" style="display: none margin:3px;">
			...
			<table width=100% align=center>
				<tbody>
					...
					<tr>
						<td>role_id:</td>
						<td><input type=text name=rolemodule.role_id id=role_id>
						</td>
						<td>中文</td>
					</tr>
					...
					<tr>
						<td>module_id:</td>
						<td><input type=text name=rolemodule.module_id id=module_id>
						</td>
						<td>中文</td>
					</tr>

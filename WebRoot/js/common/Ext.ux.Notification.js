...
Ext.ux.Notification = Ext.extend(Ext.Window, {
	...
	onDestroy : function() {
		...
		Ext.ux.NotificationMgr.positions.get(this.xpos);
	...
	animShow : function(x, y) {
		...
		do {
			...
			while (Ext.ux.NotificationMgr.positions.get(this.xpos)
					.indexOf(this.ypos) > -1)
				this.ypos++;

			// this.setSize(this.width,this.height);

			this.totalHeight = -2 - ((this.getSize().height + 10) * this.ypos);
			// 如果当前x轴上的y轴数据是第一次显示，不检测高度（如果检测高度，会导致死循环）
			if (this.ypos == 0
					|| Ext.getBody().getHeight() - this.height
							+ this.totalHeight > 0) {
				this.totalWidth = -20
						- ((this.getSize().width + 10) * this.xpos);
				Ext.ux.NotificationMgr.positions.get(this.xpos).push(this.ypos);

				this.el.alignTo(document, "br-br", [this.totalWidth,
								this.totalHeight]);
				this.el.slideIn('b', {
							duration : 1,
	...
	showSuccess : function(title, msg) {
		this.iconCls = 'x-icon-information', this.title = title || 'success';
	...
	showFailure : function(title, msg) {
		this.iconCls = 'x-icon-error', this.title = title || 'success';
		this.html = msg || 'process successfully!';
		this.show(document);
	},
	/**

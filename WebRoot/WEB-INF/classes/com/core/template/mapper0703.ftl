	...
	<select id="getByPage" parameterType="com.core.util.page.PageSelect" resultType="list" resultMap="${beanName?uncap_first}ResultMap">
		...
		*
		...
		${beanName?uncap_first} a
			<where>
		<#list fieldname as name>
					<if test="@Ognl@isNotBlank(filter.${name})">
		
			 <#if name!="${beanName?uncap_first}Id">a.${name}=<#noparse>#{</#noparse>filter.${name}<#noparse>}</#noparse>
			 <#else>a.${beanName?uncap_first}_id=<#noparse>#{</#noparse>filter.${name}<#noparse>}</#noparse>
			 </#if>
